...
interface FilippoRewardsProps {
  isFilippo?: boolean;
}

const FilippoRewards: React.FC<FilippoRewardsProps> = ({ isFilippo = false }) => {
  ...
  const rewardsData = {
    ...
    cashbackRate: 3 // 3% cashback for grupo filippo rewards
  };

  if (!isFilippo) return null;

  return (
    <div className="mx-5 mb-3 mt-3">
      <div
        className={`bg-[#7A6348] text-white rounded-xl shadow-md transition-all duration-300 ${
          isExpanded ? 'pb-3' : ''
        }`}
      >
        ...
        >
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-white/85 text-xs">Grupo Filippo Rewards</p>
            </div>
          </div>
        ...
        {isExpanded && (
          <div className="px-3 pb-2 space-y-2.5 animate-fade-in-up">
            ...
            <div className="text-white/85 text-xs px-0.5 leading-relaxed">
              ...
              <div className="pl-5">
                <span>Acumula y usa tus puntos en Grupo Filippo</span>
              </div>
            </div>
          </div>
        )}
